	...
	function filtraEst(){
		...
		<?php
			...
			if($_POST["palabra"]=="" || $_POST["categoria"]!="" || $_POST["condicion"]!=""){
				...
				$consulta="select id,'P' as tipo from publicaciones where $condicion $strCondicion UNION $consultaNat UNION $consultaJur ";
			}
			$result=$bd->query($consulta);
			$total=$result->rowCount();
			$totalPaginas=ceil($total/25);
...
//									$miTitulo=str_ireplace($_POST["palabra"], "<span style='background:#ccc'><b>" . strtoupper($_POST["palabra"]) . "</b></span>", $miTitulo);
								...
								<div class=' col-xs-12 col-sm-6 col-md-7 col-lg-7'><p class='t16 marL10 marT5'>
									...
									<br>
									<span class='t12 grisO '><i class='glyphicon glyphicon-time t14  opacity'></i> <?php echo $usua->getTiempo();?></span><br>
									<span class='t11 grisO'> <span> <i class='fa fa-eye negro opacity'></i></span><span class='marL5'>Falta</span><i class='fa fa-heart negro marL5 opacity'>
